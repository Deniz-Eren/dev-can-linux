...
#warning CONFIG_QNX_INTERRUPT_ATTACH_EVENT is deprecated; we recommend CONFIG_QNX_INTERRUPT_ATTACH
    ...
    CONFIG_QNX_INTERRUPT_MASK_PULSE == 1
#error Cannot set (CONFIG_QNX_) *INTERRUPT_MASK_ISR and *INTERRUPT_MASK_PULSE
#endif

#if CONFIG_QNX_INTERRUPT_ATTACH == 0 && ( \
        CONFIG_QNX_INTERRUPT_MASK_ISR == 1 || \
        CONFIG_QNX_INTERRUPT_MASK_PULSE == 1 )
#error Cannot set CONFIG_QNX_INTERRUPT_MASK_* without CONFIG_QNX_INTERRUPT_ATTACH
#endif

#if CONFIG_QNX_INTERRUPT_ATTACH == 1 && \
    CONFIG_QNX_INTERRUPT_MASK_ISR == 0 && \
    CONFIG_QNX_INTERRUPT_MASK_PULSE == 0
#error Must set (CONFIG_QNX_) *INTERRUPT_MASK_ISR or *INTERRUPT_MASK_PULSE when using *INTERRUPT_ATTACH
#endif
